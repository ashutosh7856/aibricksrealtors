 ...
 * Admin API Utility Functions
 * Handles all API calls for the admin panel
 * FIXED: Correct API base URL, no double slashes, production-safe
 */

/**
 * Get API base URL (always points to /api)
 */
const getApiBaseUrl = () => {
  // If env variable exists (production / external API)
  if (process.env.NEXT_PUBLIC_API_URL) {
    return process.env.NEXT_PUBLIC_API_URL.replace(/\/$/, "");
  }

  // Browser-side (same-origin)
  if (typeof window !== "undefined") {
    return `${window.location.origin}/api`;
  }
...
/**
 * Normalize endpoint (prevents double slashes)
 */
...
export const propertiesAPI = {
  getAll: async (params = {}) => {
    ...
    return await apiRequest(`/v1/properties?${query}`);
  },
...
export const contactAPI = {
  getAll: async (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return await apiRequest(`/v1/contact?${query}`);
  },
...
export const scheduleVisitAPI = {
  getAll: async (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return await apiRequest(`/v1/schedule-visit?${query}`);
  },
...
export const interestedAPI = {
  getAll: async (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return await apiRequest(`/v1/interested?${query}`);
  },
...
export const callRequestsAPI = {
  getAll: async (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return await apiRequest(`/v1/call-request?${query}`);
  },
