...
class Property {
  ...
  async getCities() {
    try {
      const snapshot = await this.collection.select('city').get();
      const cities = [...new Set(
        snapshot.docs.map(doc => doc.data().city?.trim()).filter(Boolean)
      )];
      return cities.sort();
    } catch (error) {
      logger.error('Error getting cities from properties:', error);
      return [];
    }
  }
